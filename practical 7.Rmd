...
```{r}
lm_model <- lm(price ~ carat, data = my_diamonds)
summary(lm_model)
```
...
```{r}
lm_intercept_only <- lm(price ~ 1, data = my_diamonds)
anova(lm_intercept_only, lm_model)
```
...
```{r}
my_diamonds$cut <- factor(my_diamonds$cut, ordered = FALSE)
ggplot(my_diamonds, aes(cut)) +
  geom_boxplot()
```
...
```{r}
prediction <- predict(lm_model_cut, newdata = data.frame(cut = levels(my_diamonds$cut)))
prediction_df <- data.frame(cut = levels(diamonds$cut), predicted_price = prediction)
prediction_df[which.max(prediction_df$predicted_price), ]
```
...
```{r}
ggplot(my_diamonds, aes(x = price, y = carat, color = cut)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```
...
```{r}
ggplot(Animals, aes(brain)) +
  geom_boxplot()

ggplot(Animals, aes(body)) +
  geom_boxplot()
```
...
ggplot(data = Animals, aes(x = 1, y = log_body)) +
  geom_boxplot()

ggplot(data = Animals, aes(x = 1, y = log_brain)) +
  geom_boxplot()
```
...
```{r}
ggplot(data = Animals, aes(x = log_body, y = log_brain)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE)
```
...
```{r}
m1 <- lm(log_brain ~ log_body, data = Animals)
summary(m1)
```
...
```{r}
Animals$species <- ifelse(Animals$body %in% c(11700.000, 9400.000, 87000.000), "extinct", "others")
Animals$species <- factor(Animals$species, levels = c("extinct", "others"))
```
...
```{r}
m2 <- update(m1, . ~ . + species, data = Animals)
summary(m2)
```
...
```{r}
Animals$species <- ifelse(Animals$body %in% c(11700, 9400, 87000), "extinct",
                   ifelse(Animals$body %in% c(207.000, 62.000, 52.160, 6.800), "primate", "others"))

Animals$species <- factor(Animals$species, levels = c("extinct", "primate", "others"))
```
...
```{r}
m3 <- update(m1, . ~ . + species, data = Animals)
summary(m3)
```
